...
###Main Process###########
##########################
Network = Core.Init(N,z,F,Strategy = Strategy,trianglehalfwidth=w,InitDict=InitDict)


plt.figure()
plt.plot(np.arange(len(Network)),Network)
plt.savefig(str(SaveDirName) + "/InitialOpinions.png")
plt.close()
...
for r in range(Repeats):
    Stat,HistEvolution,bin_edges = Core.Evolve(N,z,F,T,Strategy,copy.copy(Network),DistEvolution=True)
    Stats.append(Stat)
